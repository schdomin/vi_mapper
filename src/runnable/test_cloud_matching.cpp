...
int main( int argc, char** argv )
{
    ...
    //ds clouds
    std::vector< CPointCloud > vecClouds;
    ...
    //ds euclidian weight
    const double dWeightEuclidian = 1.0;

    //ds match the query clouds
    for( const CPointCloud& cCloudQuery: vecClouds )
    {
        //ds against one training cloud each
        for( const CPointCloud& cCloudTrain: vecClouds )
        {
