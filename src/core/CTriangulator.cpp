...
CTriangulator::CTriangulator( const std::shared_ptr< CStereoCamera > p_pStereoCamera,
                              ...
                              const uint32_t& p_uKeyPointSize ): m_pStereoCamera( p_pStereoCamera ),
                                                                                            m_pExtractor( p_pExtractor ),
                                                                                            m_pMatcher( p_pMatcher ),
                                                                                            m_fMatchingDistanceCutoff( p_fMatchingDistanceCutoff ),
                                                                                            m_uKeyPointSize( p_uKeyPointSize ),
                                                                                            m_uLimitedSearchRangeToLeft( 30 ),
...
const CPoint3DInCameraFrame CTriangulator::getPointTriangulatedFull( const cv::Mat& p_matImageRIGHT, const cv::KeyPoint& p_cKeyPointLEFT, const CDescriptor& p_matReferenceDescriptorLEFT ) const
{
    ...
    //ds search limit
    const uint32_t uLimit( m_pStereoCamera->m_uPixelWidth );
    ...
    if( m_fMatchingDistanceCutoff > vecMatches[0].distance )
    {
        ...
        return CMiniVisionToolbox::getPointStereoLinearTriangulationSVDLS( vecReference, vecMatch, m_pStereoCamera->m_pCameraLEFT->m_matProjection, m_pStereoCamera->m_pCameraRIGHT->m_matProjection );
    }
    else
    {
...
const CPoint3DInCameraFrame CTriangulator::getPointTriangulatedLimited( const cv::Mat& p_matImageRIGHT, const cv::KeyPoint& p_cKeyPointLEFT, const CDescriptor& p_matReferenceDescriptorLEFT ) const
{
    ...
    {
        vecPoolKeyPoints[u] = cv::KeyPoint( uBegin+u, vecReference(1), fKeyPointSize );
    }
...
const CPoint3DInCameraFrame CTriangulator::getPointTriangulatedLimited( const cv::Mat& p_matImageRIGHT, const cv::Point2d& p_ptReferenceLEFT, const CDescriptor& p_matReferenceDescriptorLEFT ) const
{
    //ds left reference x
    const int32_t iUReference( p_ptReferenceLEFT.x );
    ...
    for( uint32_t u = 0; u < m_uLimitedSearchRange; ++u )
    {
        vecPoolKeyPoints[u] = cv::KeyPoint( uBegin+u, p_ptReferenceLEFT.y, m_uKeyPointSize );
    }
    ...
    {
        //ds triangulate 3d point
        return CMiniVisionToolbox::getPointStereoLinearTriangulationSVDLS( p_ptReferenceLEFT,
                                                                           vecPoolKeyPoints[vecMatches[0].trainIdx].pt,
                                                                           m_pStereoCamera->m_pCameraLEFT->m_matProjection,
                                                                           m_pStereoCamera->m_pCameraRIGHT->m_matProjection );
    }
    else
    {
        throw CExceptionNoMatchFound( "matching distance: " + std::to_string( vecMatches[0].distance ) );
